...
>(({ className, children, ...props }, ref) => (
  <DialogPortal>
    ...
    >
      {children}
      <DialogPrimitive.Close className="absolute right-4 top-4 z-10 rounded-full bg-surface-2/80 p-2 hover:bg-surface-2 transition-all">
      ...
      <TooltipProvider>
        ...
        >
          <div
            ref={ref}
            className={cn(
              "rounded-3xl border border-border bg-popover p-2 shadow-lg transition-all duration-300 w-full",
...
export const PromptInputBox = React.forwardRef((props: PromptInputBoxProps, ref: React.Ref<HTMLDivElement>) => {
  ...
  const processFile = (file: File) => {
    ...
    if (file.size > 10 * 1024 * 1024) {
      console.log("Archivo muy grande (máximo 10MB)");
      return;
    }
    setFiles([file]);
  ...
  return (
    <StarBorder 
      ...
      color="#33C7D8" 
      speed="5s" 
      className={cn("w-full relative z-50 p-[1px]", className)}
    >
      <PromptInput
        value={input}
        ...
        className={cn(
          "w-full bg-transparent shadow-none transition-all duration-300 ease-in-out !p-0 !border-0",
      ...
      >
        {files.length > 0 && !isRecording && (
          <div className="flex flex-wrap gap-2 px-3 pt-3 transition-all duration-300">
            {files.map((file, index) => (
              <div key={index} className="relative group">
                {file.type.startsWith("image/") && filePreviews[file.name] && (
                  <div
                    className="w-16 h-16 rounded-xl overflow-hidden cursor-pointer transition-all duration-300 border border-border"
        ...
        <div className="flex items-end w-full px-3 py-2">
          {/* Attachment button on the left */}
          <div
            className={cn(
              "flex items-center gap-1 transition-opacity duration-300 mr-2 pb-1",
              isRecording ? "opacity-0 invisible w-0" : "opacity-100 visible w-8"
            )}
          >
            <PromptInputAction tooltip="Adjuntar imagen">
              <motion.button
                ...
                className={cn(
                  ...
                  hasFiles ? "text-primary" : "text-muted-foreground hover:bg-surface-2 hover:text-foreground"
                )}
          ...
          <div className="flex items-center ml-2 pb-1">
            ...
            >
              <Button
                ...
                size="icon"
                className={cn(
                  "h-8 w-8 rounded-full transition-all duration-200 shadow-none border-none flex items-center justify-center",
                  isRecording
                    ? "bg-transparent hover:bg-surface-2 text-destructive hover:text-destructive"
                    : hasContent
                    ? "bg-transparent hover:bg-surface-2 text-[#33C7D8]"
